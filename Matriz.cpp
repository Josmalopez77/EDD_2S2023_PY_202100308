...
void Matriz::Generar_JSON(Cola2 *cola){
    cola->VerProyectos();
    NodoCola2 *aux = cola->Primero;

    json proyectosJson;
    while(aux){
        json proyectoJson;
        proyectoJson["Prioridad"] = aux->Prioridad;
        NodoListaDoble *auxT = this->Tareas->Primero;
        while(auxT){
            json Tareas;
            if(aux->Proyecto_C->Codigo==auxT->Codigo){
                Tareas["nombre"] = auxT->Nombre_Tarea;
                Tareas["Empleado"]= auxT->Codigo_Encargado;
                proyectoJson["Tareas"].push_back(Tareas);
            }
            auxT = auxT->Siguiente;
        }
        proyectoJson["id"] = aux->Proyecto_C->Codigo;
        proyectoJson["nombre"] = aux->Proyecto_C->Nombre;
        proyectosJson["Proyectos"].push_back(proyectoJson);
        aux = aux->Siguiente;

    }
//proyectosJson["Proyectos"].push_back(proyectosJson);

    std::ofstream archivo("Prueba.json");
    if (archivo.is_open()) {
        archivo << proyectosJson.dump(4); // 4 para indentación
        archivo.close();
        std::cout << "Archivo JSON generado correctamente." << std::endl;
    } else {
...
void Matriz::Insertar_Tareas(ListaDoble *Tareas){
    this->Tareas = Tareas;
}
